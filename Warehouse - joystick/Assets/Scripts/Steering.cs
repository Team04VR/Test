...
public class Steering : MonoBehaviour {
	...
	void Update () { 
		...
		if(InputBroker.GetKeyDown(WiimoteName + ":Up")){
			...
			//GameObject.Find("Graphics").transform.position+=transform.forward * TravelSpeed * Time.deltaTime;
		}
...
//			GameObject.Find("Graphics").transform.rotation.y-= RotationSpeed * Time.deltaTime;
		}
		if(InputBroker.GetKeyDown(WiimoteName + ":A")) {
			CommonVariables.mappedPosition += transform.up * TravelSpeed * Time.deltaTime;
		}
